...
class ExperimentPlanGenerator:
    def __init__(self, planning_queue: queue.Queue, base_env: Environment, options, glue_code, fm):
    ...
    def _create_next_environments(self, base_env: Environment):
        to_explore = []
    ...
    def run(self):
        ...
        elif type_plan == "CRITERIA":
            self._create_next_environments(self.base_environment)
        elif type_plan in ("DAY", "HOURS", "MINUTES"):
            cutt_of_format = "TIME"
            deadline: datetime = self.options.get_cutt_of_planning_h()
